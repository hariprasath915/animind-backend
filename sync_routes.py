...
# ══════════════════════════════════════════════════════════════════════
# ASSIGNMENTS & 7-HOUR EXPIRATION SYSTEM
# ══════════════════════════════════════════════════════════════════════
...
# Backend in-memory store (thread-safe fallback + Supabase table synchronization)
_ASSIGNMENTS_STORE = {}
_SUBMISSIONS_STORE = {}
...
class JoinAssignmentRequest(BaseModel):
...
def create_assignment(payload: CreateAssignmentRequest, current_user: dict = Depends(get_current_user)):
    """
    Teacher/Admin: Create a new assignment with a strict 7-HOUR server expiration.
    Generates a unique 6-character code (e.g. ABC123).
    """
    now_dt = datetime.now(timezone.utc)
    expires_dt = now_dt + timedelta(hours=7)
    
    code = "".join(random.choices(string.ascii_uppercase + string.digits, k=6))
    assign_id = f"asgn_{secrets.token_hex(6)}"
    
    teacher_id = current_user.get("id", "")
    teacher_name = payload.teacher_name or current_user.get("name", "Teacher")
    institution = payload.institution or current_user.get("institution", "Institution")
    ...
    record = {
        ...
        "content_id": payload.content_id or "",
        "institution": institution,
        "teacher_id": teacher_id,
        "teacher_name": teacher_name,
        "created_at": now_dt.isoformat(),
        "expires_at": expires_dt.isoformat(),
        "expires_in_seconds": 25200,
        "status": "active"
    }
    
    _ASSIGNMENTS_STORE[code] = record
    
    try:
        sb = get_supabase()
        sb.table("assignments").insert(record).execute()
    except Exception as e:
        print(f"[ASSIGNMENTS] DB mirror note: {e}")
        
    print(f"[ASSIGNMENTS] ✅ Created: {code} for '{payload.title}' (expires at {expires_dt.isoformat()})")
    
    return {
        "success": True,
        "assignment": record,
        "code": code,
        "share_link": f"/join?code={code}",
        "expires_at": expires_dt.isoformat(),
        "message": f"Assignment created successfully! Code: {code}. Expires in 7 hours."
    }


@router.post("/assignments/join", status_code=200)
def join_assignment(payload: JoinAssignmentRequest, current_user: dict = Depends(get_current_user)):
    """
    Student: Validate assignment code and check 7-HOUR expiration on server.
    """
    code = payload.code.strip().upper()
    now_dt = datetime.now(timezone.utc)
    
    record = _ASSIGNMENTS_STORE.get(code)
    
    if not record:
        try:
            sb = get_supabase()
            res = sb.table("assignments").select("*").eq("code", code).maybe_single().execute()
            if res and res.data:
                record = res.data
                _ASSIGNMENTS_STORE[code] = record
        except Exception:
    ...
    if not record:
        raise HTTPException(status_code=404, detail="Invalid assignment code. Please check the code and try again.")
        
    # STRICT 7-HOUR EXPIRATION CHECK
    exp_dt = datetime.fromisoformat(record["expires_at"].replace("Z", "+00:00"))
    if now_dt >= exp_dt:
        record["status"] = "expired"
        raise HTTPException(
            status_code=410,
            detail="This assignment has expired. Expirations occur 7 hours after creation."
        )
...
@router.post("/assignments/submit", status_code=200)
def submit_assignment(payload: SubmitAssignmentRequest, current_user: dict = Depends(get_current_user)):
    """
    Student: Submit assignment results. Strictly enforces 7-hour server expiration.
    """
    code = payload.code.strip().upper()
    now_dt = datetime.now(timezone.utc)
    ...
    if not record:
        raise HTTPException(status_code=404, detail="Invalid assignment code.")

    exp_dt = datetime.fromisoformat(record["expires_at"].replace("Z", "+00:00"))
    if now_dt >= exp_dt:
        raise HTTPException(
            status_code=410,
            detail="Cannot submit. This assignment has expired."
        )
    ...
    if code not in _SUBMISSIONS_STORE:
        _SUBMISSIONS_STORE[code] = []
    _SUBMISSIONS_STORE[code].append(sub_record)
    
    try:
        sb = get_supabase()
        sb.table("assignment_submissions").insert(sub_record).execute()
    except Exception as e:
        print(f"[SUBMISSIONS] DB mirror note: {e}")

    print(f"[SUBMISSIONS] ✅ Student {payload.student_name} ({payload.roll_number}) submitted {code}: {payload.score}%")

    return {
        "success": True,
        "submission": sub_record,
        "message": f"Assignment submitted successfully! Score: {payload.score}%"
    }


@router.get("/assignments/teacher/list", status_code=200)
def list_teacher_assignments(current_user: dict = Depends(get_current_user)):
    """
    Teacher: List created assignments with 7-hour active/expired status and submission counts.
    """
    now_dt = datetime.now(timezone.utc)
    teacher_id = current_user.get("id", "")
    
    assignments = list(_ASSIGNMENTS_STORE.values())
    
    result = []
    for a in assignments:
        if teacher_id and a.get("teacher_id") and a.get("teacher_id") != teacher_id:
            continue
        exp_dt = datetime.fromisoformat(a["expires_at"].replace("Z", "+00:00"))
        is_active = now_dt < exp_dt
        subs = _SUBMISSIONS_STORE.get(a["code"], [])
        
        avg_score = round(sum(s["score"] for s in subs) / len(subs), 1) if subs else 0.0
...
@router.get("/assignments/student/my-assignments", status_code=200)
def list_student_assignments(current_user: dict = Depends(get_current_user)):
    """
    Student: List accessible assignments with live 7-hour status and completed scores.
    """
    now_dt = datetime.now(timezone.utc)
    student_id = current_user.get("id", "")
    
    result = []
    for code, a in _ASSIGNMENTS_STORE.items():
        exp_dt = datetime.fromisoformat(a["expires_at"].replace("Z", "+00:00"))
        is_active = now_dt < exp_dt
        subs = _SUBMISSIONS_STORE.get(code, [])
        user_sub = next((s for s in subs if s.get("student_id") == student_id), None)
        
        if user_sub:
            status = "Completed"
            score = user_sub.get("score")
        elif not is_active:
            status = "Expired"
            score = None
        else:
        ...
        result.append({
            ...
            "title": a["title"],
            "subject": a["subject"],
            "teacher_name": a.get("teacher_name", "Faculty"),
            "expires_at": a["expires_at"],
            "is_active": is_active,
            "status": status,
            "score": score
    ...
    return {"my_assignments": result, "count": len(result)}
