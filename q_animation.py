...
=============================================================
v0.5 -- NEW ARCHITECTURE (backward-compatible with v0.4):

  NEW IN v0.5:
  - Redesigned model routing:
      Gemini        → Scene 0 only  (visual hook / concept intro)
      Claude Sonnet → Scenes 1 to N (solution flow) — ONE call
      Claude Haiku  → Step solution + analyzers (unchanged)
  - Stage 1 (_generate_concept_animation) REMOVED — Gemini owns Scene 0.
  - Stage 2 (Solution AI) now generates scenes 1..N only (skips Scene 0).
    The Sonnet prompt instructs Claude to start at Scene 1 and leave
    Scene 0 as a placeholder for Gemini to fill in.
  - Gemini splicing logic unchanged — still patches scene-0 <g> + animateScene0().
  - concept_animation_code in the result now returns the solution animation
    (same as animation_code) since the dedicated concept stage is removed.
  - Engine version bumped to "v0.5".
...
GEMINI_MODEL = "gemini-3.1-pro-preview"  # Gemini 3.1 Pro (correct API model string)
_gemini_client = None
...
def _build_prompt(question, category, n_scenes: int = 5, enrichment: str = ""):
    ...
    last_scene = n_scenes - 1
    animate_range = f"animateScene1-{last_scene}"
    user_content = (
        f"Build a PREMIUM CINEMATIC {n_scenes}-SCENE SVG ANIMATION for QAnim v0.5.\n\n"
        f"QUESTION: {question}\n"
        f"CATEGORY: {category}\n"
        f"STRATEGY: {strategy}\n\n"
        f"KEY REMINDERS v0.5 (NEW ARCHITECTURE):\n"
        ...
        f"- CRITICAL v0.5: Scene 0 is generated by Gemini (visual hook). YOU must still include\n"
        f"  a minimal <g id='scene-0' class='scene'> placeholder in your SVG so the JS works.\n"
        f"  The placeholder will be REPLACED by Gemini's Scene 0 at runtime.\n"
        f"  Placeholder scene-0 can be a simple light-background rect with loading text.\n"
        f"- YOUR REAL WORK is Scenes 1 to {last_scene} (solution flow only).\n"
        f"  Scene 1: 'The Big Idea' — introduce the key concept visually.\n"
        f"  Scenes 2..{last_scene-1}: progressive solution steps, one concept per scene.\n"
        f"  Scene {last_scene} (LAST): summary / step-by-step method only.\n"
        f"- JS pattern: buildDots + showScene + fadeIn + dashIn + animateScene0 (stub) + "
        f"{animate_range} + DOMContentLoaded\n"
        f"- animateScene0() must be defined as a stub (it will be overwritten by Gemini's version).\n"
        f"- solution_steps: {n_scenes} SHORT, VERY SIMPLE English sentences (one per scene), not numerical "
        f"working. Use easy daily words, short sentences, and a friendly teacher tone.\n"
        ...
        f"'Substitute values', 'Solve'). NEVER show any computed numerical answer.\n"
...
async def _run_generation_pipeline(question):
    """
    PIPELINE v0.5 — NEW ARCHITECTURE:
    ...
    Stage 0.5 -- SimpleMethodAnalyzer + SmartPromptBuilder (claude-haiku-4-5, concurrent)
    Stage 1   -- [REMOVED] Concept Animation stage eliminated.
                 Gemini now owns Scene 0 exclusively.
    Stage 2   -- Solution Animation  (claude-sonnet-4-6, Scenes 1..N only, ONE call)
    Stage 3   -- Haiku Solution      (claude-haiku-4-5)
    Stage G   -- Gemini Scene 0      (gemini-2.0-flash, concurrent with Stage 2 & 3)
               [Stages 0.5, 2, 3, G concurrent via asyncio.gather]

    Post-processing builds the full v0.2 page layout from the AI SVG,
    then splices Gemini's Scene 0 into the final HTML.
    ...
    """
    short_q = question[:80] + ("..." if len(question) > 80 else "")
    QAnimLogger.info("Pipeline", f"START v0.5 -- '{short_q}'")
    ...
    async def _run_all_stages():
        ...
        # ── Sub-stage B: launch Solution AI + Haiku + Gemini Scene 0 concurrently ──
        # v0.5 NEW ARCHITECTURE:
        #   - Stage 1 (concept animation) is REMOVED — Gemini owns Scene 0.
        #   - Stage 2 (Solution AI / Claude Sonnet) generates Scenes 1..N only.
        #   - Stage G (Gemini) generates Scene 0 concurrently.
        #   - Stage 3 (Haiku) generates step solution concurrently.
        # All stages receive the enrichment so every AI stage has full analysis context.
        ...
        # [v0.5] No concept_fut — Stage 1 removed. Gemini handles Scene 0.
        QAnimLogger.info("Gemini", "Generating Scene 0 concurrently with Solution AI + Haiku...")
        gemini_fut = asyncio.ensure_future(
            GeminiScene1Generator.generate_async(ai_question, category, n_scenes)
        )

        # Solution AI + Haiku + Gemini all run concurrently
        sol_raw, haiku_sol, (gemini_scene0_group, gemini_animate0_fn) = await asyncio.gather(
            _run_solution_ai(),
            HaikuSolutionGenerator.generate_async(ai_question, haiku_hint),
            gemini_fut,
        )

        # [v0.5] concept_html is not generated separately; use empty string as placeholder.
        # The caller will set concept_animation_code = animation_code after page build.
        concept_html = ""
    ...
    result["concept_animation_code"] = concept_html  # empty until page build; set below
    ...
    result["concept_animation_code"] = html   # v0.5: no separate concept stage; alias to solution
    result["render_status"]          = "ok"

    QAnimLogger.ok("Pipeline", (
        f"DONE v0.5 -- '{result['title']}' "
        f"solution={len(html):,} "
...
if __name__ == "__main__":
    ...
    for cat, q in questions_to_test.items():
        print("=" * 72)
        print(f"  QAnim v0.5 -- New Architecture | {cat}")
        print(f"  Q: {q[:65]}...")
        print("=" * 72)
        ...
        print(f"[Stage 1] REMOVED (Gemini owns Scene 0 in v0.5)")
        print(f"[Stage 2] Solution  : {len(solution_html):,} chars  (Scenes 1..N, Claude Sonnet)")
        ...
        slug = cat.lower()
        solution_out = f"q_anim_v05_{slug}_solution.html"

        with open(solution_out, "w", encoding="utf-8") as f: f.write(solution_html)

        print(f"\n[Stage 2] Solution saved : {solution_out}")
        print()
        print("v0.5 Architecture (Gemini Scene 0 + Claude Sonnet Scenes 1..N):")
        print("  OK  Gemini → Scene 0 (visual hook / concept intro)")
        print("  OK  Claude Sonnet → Scenes 1..N (solution flow, ONE call)")
        print("  OK  Claude Haiku → Step solution + analyzers")
