  ...
  function s6Render(){
    ...
    if(nb){
      if(s6Phase<n){
        ...
        nb.onclick=function(){window.qanim_showScene7();};
        if(!s6AutoTimer){
          s6AutoTimer=setTimeout(function(){
            var ov=_el('qanim-scene6-overlay');
            if(ov&&ov.classList.contains('qanim-scene-visible')) window.qanim_showScene7();
          },3200);
        }
      }
    }
...
(function initScene7(){
  ...
  window.qanim_showScene7=window.qanim_showScene7;
})();
</script>"""
...
(function(){
  ...
  var _SHOW_FN_NAMES=['qanim_showScene6','qanim_showScene7',
  ...
  var _OVERLAY_IDS=['qanim-scene6-overlay','qanim-scene7-overlay',
                    'qanim-scene9-overlay'];
  ...
  function _nextStep(){
    ...
    if(cs>=ts){
      ...
      // Restore SVG visibility if it was faded by a previous panel trigger
      var svgC=document.querySelector('.svg-container');
      if(svgC&&parseFloat(svgC.style.opacity||'1')<1){
        svgC.style.transition='none'; svgC.style.opacity='1';
      }
      var nxt=cs+1;
...
(function(){
  ...
  var names=['qanim_showScene6','qanim_showScene6','qanim_showScene7','qanim_showScene9',
...
// ── prevStep: restore SVG and close any open panels ──────────────────────
function prevStep() {
  if (currentStep > 0) {
    // Close any open scene overlays
    ...
    // Restore SVG visibility (may have been faded when panel opened)
    var svgC = document.querySelector('.svg-container');
    if (svgC) { svgC.style.transition = 'none'; svgC.style.opacity = '1'; }
    currentStep--;
    window.currentStep = currentStep;
    applyStep(currentStep);
  }
...
// ── goToStep: jump to any SVG step, restoring SVG visibility ─────────────
function goToStep(idx) {
  if (idx >= 0 && idx <= totalSteps) {
    ...
    // Restore SVG container (was faded to 0 when formula panel opened)
    var svgCont = document.querySelector('.svg-container');
    if (svgCont) { svgCont.style.transition = 'none'; svgCont.style.opacity = '1'; }
    currentStep = idx;
    window.currentStep = idx;
    applyStep(currentStep);
  }
...
<script async src="https://cdn.jsdelivr.net/npm/mathjax@3/es5/tex-svg.js"></script>"""
    ...
    def run_all_passes(cls, html: str, scene_script: dict) -> str:
        ...
        html = cls.upgrade_html_badges(html)           # FIX B1: badges backtick fix
        html = cls._fix_goto_step(html)                # FIX B2+B3: goToStep window + svgCont
        html = cls._fix_prev_step(html)                # FIX B4: prevStep svgCont restore
...
class MathTypography:
    ...
    @staticmethod
    def _fix_goto_step(html: str) -> str:
        """
        FIX B2+B3: goToStep must (a) be exposed on window, and (b) restore
        svgCont opacity + close scene overlays before navigating.
        When the formula panel opens, svgCont is faded to opacity:0.
        If user then clicks a step dot, svgCont stays invisible unless reset.
        This regex patch replaces the Gemini-generated goToStep with a
        robust version that handles the svgCont restore and overlay close.
        """
        FIXED = (
            'function goToStep(idx){'
            'if(idx<0||idx>window.totalSteps)return;'
            "['qanim-scene6-overlay','qanim-scene7-overlay','qanim-scene9-overlay'].forEach(function(id){"
            'var o=document.getElementById(id);if(o)o.classList.remove(\'qanim-scene-visible\');});'
            "var bd=document.getElementById('qanim-scene-modal-backdrop');"
            'if(bd)bd.classList.remove(\'qanim-scene-visible\');'
            "var sv=document.querySelector('.svg-container');"
            "if(sv){sv.style.transition='none';sv.style.opacity='1';}"
            'window.currentStep=idx;'
            "if(typeof window.applyStep==='function')window.applyStep(idx);"
            '}'
            'window.goToStep=goToStep;'
        )
        # Replace any existing goToStep definition + window.goToStep assignment
        import re as _re
        patched = _re.sub(
            r'function goToStep\s*\([^)]*\)\s*\{[^}]+\}'
            r'(?:\s*window\.goToStep\s*=\s*goToStep\s*;)?',
            FIXED,
            html
        )
        # If no replacement happened, append window.goToStep = goToStep after function
        if patched == html and 'window.goToStep' not in html and 'function goToStep' in html:
    ...
    @staticmethod
    def _fix_prev_step(html: str) -> str:
        """
        FIX B4+B7: prevStep must restore svgCont opacity and close scene
        overlays before stepping back. Also ensures window.prevStep is set.
        """
        # Only patch the Gemini-generated prevStep (not master controller's)
        # The master controller's window.prevStep runs at DOMContentLoaded and
        # overrides this anyway, but we fix it here as belt-and-suspenders.
        import re as _re
        # Check if window.prevStep already has our overlay-close logic
        if 'qanim-scene-visible' in html[html.find('window.prevStep'):html.find('window.prevStep')+400]:
            return html  # already fixed by master controller
        # Add window.prevStep = prevStep after function definition if missing
        if 'function prevStep' in html and 'window.prevStep=prevStep' not in html and 'window.prevStep = prevStep' not in html:
            html = _re.sub(
                r'(function prevStep\s*\([^)]*\)\s*\{[^}]+\})',
                r'\1\n    window.prevStep = prevStep;',
                html, count=1
            )
        return html
    ...
    def upgrade_html_badges(cls, html: str) -> str:
        """
        FIX B1: badges: '<span class='badge ...'>' uses unescaped single quotes
        inside a single-quoted JS string → SyntaxError: Unexpected identifier 'badge'
        Fix: convert to backtick template literals so inner single quotes are safe.
        Pattern: badges: '<span ...>' → badges: `<span ...>`
        """
        import re as _re
        def _fix_badge(m):
            inner = m.group(1)
            # Only fix if inner content contains single quotes (the source of the bug)
            if "'" in inner or 'class=' in inner:
                return 'badges: `' + inner + '`'
            return m.group(0)
        # Match: badges: '<anything>' (single-quoted string containing HTML)
        html = _re.sub(r"badges:\s*'(<span[^`]+?</span>)'", _fix_badge, html)
        # Also fix badges: '<span class='...'>' where inner quotes break parsing
        html = _re.sub(
            r"badges:\s*'(<span\s[^']*(?:'[^']*'[^']*)*>.*?</span>)'",
            lambda m: "badges: `" + m.group(1) + "`",
