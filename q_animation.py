...
html,body{{width:100%;height:100%;background:#0b0c10;
  font-family:-apple-system,'Segoe UI',Arial,sans-serif;
  display:flex;align-items:center;justify-content:center;color:#c5c6c7}}
.card{{background:#1f2833;border:1px solid #333;border-radius:16px;
  box-shadow:0 4px 24px rgba(0,0,0,.5);padding:36px 40px;max-width:520px;text-align:center}}
.icon{{font-size:40px;margin-bottom:16px}}
.title{{font-size:17px;font-weight:800;color:#fff;margin-bottom:10px}}
.reason{{font-size:11px;color:#66fcf1;background:#0b0c10;border-radius:10px;
  padding:10px 14px;margin:12px 0;border:1px solid #333;text-align:left;
  line-height:1.6;font-family:monospace}}
.question{{font-size:12px;color:#888;line-height:1.6;margin-top:10px;font-style:italic}}
.retry-hint{{margin-top:18px;font-size:11px;font-weight:700;letter-spacing:1.5px;
  text-transform:uppercase;color:#45a29e}}
</style></head><body>
<div class="card">
  ...
  <div class="dashboard">
    ...
    <div class="svg-container">
      <svg id="stage" viewBox="0 0 850 478" preserveAspectRatio="xMidYMid slice">
        ...
        <!-- blur-shield rect -->
        <rect id="blur-shield" width="100%" height="100%" fill="#050508" opacity="0" pointer-events="none" />
...
2. Dark background: radial-gradient(circle at center, #1a1a24 0%, #050508 100%).
...
CRITICAL REMINDERS:
1. Follow the reference output style EXACTLY (dark dashboard, SVG canvas, control panel).
        ...
        <div class="svg-container">
            <svg id="stage" viewBox="0 0 850 478" preserveAspectRatio="xMidYMid slice">
                ...
                <g class="svg-layer" id="layer-frame">
                    <line x1="100" y1="239" x2="750" y2="239" stroke="#333" stroke-width="1" stroke-dasharray="8,6"/>
                    <text x="425" y="50" fill="#66fcf1" font-size="20" font-weight="bold" text-anchor="middle" filter="url(#glowCyan)">{html_module.escape(script.get('title', title))}</text>
                </g>
                <!-- Blur shield -->
                <rect id="blur-shield" width="100%" height="100%" fill="#050508" opacity="0" class="svg-layer" pointer-events="none" />
                <!-- Step overlays -->
                {overlays_html}
            </svg>
        </div>
...
if __name__ == "__main__":
    ...
    for cat, q in questions_to_test.items():
        ...
        print("  OK  Dark dashboard style (reference output pattern)")
