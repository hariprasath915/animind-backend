...
<div id="answerbox-panel" role="dialog" aria-label="Answer Box" aria-hidden="true">
  ...
  <div class="ab-body">
    ...
    <div id="ab-alldone-card">
      ...
      <div class="ab-alldone-sub">Great work! Review the animation to see the full solution.</div>
    </div>
  </div>
</div>
</div>
...
# ===========================================================================
#  MODULE 13.5 — SCENE 7: "The Big Idea" + SCENE 8: "How We Solve It"
#  -----------------------------------------------------------------------
#  These two scenes are injected AFTER all the existing SVG animation scenes
#  (scenes 1–6). They appear as full-panel HTML "cards" below the dashboard,
#  revealed when the user clicks dedicated step dots added to the control
#  panel.
#
#  Scene 7 — "The Big Idea"
#    • Shows ONE main formula / law that unlocks the problem.
#    • Labels each variable with plain-language meaning via coloured chips.
#    • Arrows point from each variable chip up to the formula box.
#    • Design mirrors reference images: large formula box, coloured variable
#      cards underneath connected by diagonal lines, insight footer strip.
#
#  Scene 8 — "How We Solve It — Step by Step"
#    • Numbered steps (5–10), each in a coloured rounded card.
#    • Title (bold) + equation / working (regular weight).
#    • Final numeric answer is intentionally withheld ("Try it yourself!").
#    • Design mirrors reference images: alternating card colours, step number
#      in large coloured numeral on the left, content on the right.
#
#  Both scenes:
#    • Are generated by Gemini (one JSON call per scene, concurrent with the
#      existing pipeline stages).
#    • Degrade gracefully to a "placeholder" if Gemini is unavailable.
#    • Are injected as a separate <div class="dashboard"> block below the
#      existing animation dashboard, so they never interfere with the SVG
#      canvas or control panel layout.
#    • Add two new pill dots ("The Big Idea", "Step by Step") to the existing
#      step-indicator via JS so they appear inline with existing dots.
# ===========================================================================

# ---------------------------------------------------------------------------
#  Prompts for Scene 7 + Scene 8 generation
# ---------------------------------------------------------------------------

_BIG_IDEA_SYSTEM = """You are an expert physics/engineering teacher. Given a student question, identify the ONE core formula or law that unlocks the problem.

Return ONLY valid JSON — no markdown, no fences, no preamble:
{
  "formula_name": "Short descriptive name, e.g. 'Newton's Second Law' or 'Ohm's Law'",
  "formula_display": "The formula as plain text, e.g. 'F = m x a' or 'Q = h x A x DeltaT'",
  "formula_insight": "One sentence explaining what this formula means physically, max 20 words",
  "variables": [
    {
      "symbol": "F",
      "color": "blue",
      "label": "Short variable name, e.g. 'Force'",
      "meaning": "Plain English meaning, max 8 words, e.g. 'Net force on the object (N)'"
    }
  ],
  "key_note": "One memorable insight about this formula, e.g. 'Bigger mass = smaller acceleration for the same force!'"
}

Rules:
- variables: list EVERY symbol in the formula. Maximum 6 variables.
- color: assign one of these per variable (cycle if more than 4): blue, orange, green, pink
- blue=#2563eb, orange=#d97706, green=#16a34a, pink=#db2777
- formula_display: use plain ASCII, e.g. 'x' for multiply, '^2' for squared, 'sqrt()' for square root
- Return raw JSON only — starts with { ends with }"""

_BIG_IDEA_USER = "Question: {question}\n\nReturn ONLY the JSON."

_STEP_BY_STEP_SYSTEM = """You are an expert engineering professor. Given a student question, write a concise step-by-step solution guide showing the METHOD — NOT the final numerical answer.

Return ONLY valid JSON — no markdown, no fences, no preamble:
{
  "steps": [
    {
      "title": "Short bold action, e.g. 'Write the governing formula'",
      "equation": "The formula or expression for this step, e.g. 'a = g sin(theta) / (1 + I/MR^2)'"
    }
  ],
  "try_it_note": "Encourage-the-student note for the last step, e.g. 'Ratio = a_solid / a_hollow = ? (Try it yourself!)'"
}

Rules:
- steps: 5–10 steps, ordered logically. Show the METHOD clearly without computing the final number.
- Last step must describe computing the final answer but NOT show the numeric result.
- equation: write each equation/expression in plain ASCII text.
- Return raw JSON only — starts with { ends with }"""

_STEP_BY_STEP_USER = "Question: {question}\n\nReturn ONLY the JSON."


# ---------------------------------------------------------------------------
#  Gemini generators for Scene 7 + Scene 8
# ---------------------------------------------------------------------------

class GeminiBigIdeaGenerator:
    """Generates Scene 7 'The Big Idea' content from a question."""

    _FALLBACK = {
        "formula_name": "Core Governing Formula",
        "formula_display": "Output = f(Inputs)",
        "formula_insight": "This formula directly relates the unknown to the given quantities.",
        "variables": [
            {"symbol": "Output", "color": "blue",   "label": "What we seek",   "meaning": "The quantity to find"},
            {"symbol": "f",      "color": "orange",  "label": "The law",        "meaning": "The governing relationship"},
            {"symbol": "Inputs", "color": "green",   "label": "Given values",   "meaning": "The known quantities"},
        ],
        "key_note": "Always identify the governing law before substituting numbers!",
    }

    @classmethod
    def generate(cls, question: str) -> dict:
        if _gemini_client is None:
            QAnimLogger.warn("BigIdeaGen", "Gemini unavailable — using fallback")
            return cls._FALLBACK
        try:
            prompt = _BIG_IDEA_USER.replace("{question}", question[:800])
            raw = GeminiSolutionGenerator._call_gemini(prompt, _BIG_IDEA_SYSTEM, max_tokens=1200)
            raw = _sanitize_json_str(raw)
            data = json.loads(raw)
            # Validate minimum fields
            if data.get("formula_name") and data.get("formula_display") and data.get("variables"):
                QAnimLogger.ok("BigIdeaGen", f"Big Idea generated: {data['formula_name']}")
                return data
            QAnimLogger.warn("BigIdeaGen", "Incomplete response — using fallback")
        except Exception as e:
            QAnimLogger.warn("BigIdeaGen", f"Failed: {e} — using fallback")
        return cls._FALLBACK
...
class GeminiStepByStepGenerator:
    """Generates Scene 8 'How We Solve It' content from a question."""

    _FALLBACK = {
        "steps": [
            {"title": "Identify the given values",          "equation": "List all known quantities with their units"},
            {"title": "Write the governing formula",         "equation": "Select the appropriate law or equation"},
            {"title": "Rearrange for the unknown",           "equation": "Isolate the target variable algebraically"},
            {"title": "Substitute the given values",         "equation": "Insert numerical values (keep units)"},
            {"title": "Compute the result",                  "equation": "Evaluate the expression — answer = ?"},
        ],
        "try_it_note": "Substitute the numbers and compute the final answer yourself!",
    }

    @classmethod
    def generate(cls, question: str) -> dict:
        if _gemini_client is None:
            QAnimLogger.warn("StepByStepGen", "Gemini unavailable — using fallback")
            return cls._FALLBACK
        try:
            prompt = _STEP_BY_STEP_USER.replace("{question}", question[:800])
            raw = GeminiSolutionGenerator._call_gemini(prompt, _STEP_BY_STEP_SYSTEM, max_tokens=2000)
            raw = _sanitize_json_str(raw)
            data = json.loads(raw)
            if data.get("steps") and len(data["steps"]) >= 3:
                QAnimLogger.ok("StepByStepGen", f"Step-by-step generated: {len(data['steps'])} steps")
                return data
            QAnimLogger.warn("StepByStepGen", "Incomplete response — using fallback")
        except Exception as e:
            QAnimLogger.warn("StepByStepGen", f"Failed: {e} — using fallback")
        return cls._FALLBACK
...
# ---------------------------------------------------------------------------
#  HTML builders for Scene 7 + Scene 8 cards
# ---------------------------------------------------------------------------

# Colour palette for variable chips (matches reference images)
_VAR_COLORS = {
    "blue":   {"bg": "rgba(37,99,235,0.09)",  "border": "rgba(37,99,235,0.30)",  "text": "#1d4ed8", "arrow": "#2563eb"},
    "orange": {"bg": "rgba(217,119,6,0.09)",  "border": "rgba(217,119,6,0.30)",  "text": "#92400e", "arrow": "#d97706"},
    "green":  {"bg": "rgba(22,163,74,0.09)",  "border": "rgba(22,163,74,0.30)",  "text": "#15803d", "arrow": "#16a34a"},
    "pink":   {"bg": "rgba(219,39,119,0.09)", "border": "rgba(219,39,119,0.30)", "text": "#9d174d", "arrow": "#db2777"},
}
_VAR_COLOR_ORDER = ["blue", "orange", "green", "pink"]

# Step card colours for Scene 8 (match reference images)
_STEP_CARD_COLORS = [
    {"bg": "rgba(219,39,119,0.07)",  "border": "#db2777", "num": "#db2777"},   # pink/red
    {"bg": "rgba(217,119,6,0.07)",   "border": "#d97706", "num": "#d97706"},   # orange
    {"bg": "rgba(22,163,74,0.07)",   "border": "#16a34a", "num": "#16a34a"},   # green
    {"bg": "rgba(37,99,235,0.07)",   "border": "#2563eb", "num": "#2563eb"},   # blue
    {"bg": "rgba(124,58,237,0.07)",  "border": "#7c3aed", "num": "#7c3aed"},   # purple
    {"bg": "rgba(14,116,144,0.07)",  "border": "#0e7490", "num": "#0e7490"},   # cyan
]

_SCENES_78_CSS = """
<style id="qanim-scenes78-styles">
/* ── Shared scene card shell ── */
.qanim-scene-card {
  width: 100%;
  max-width: 900px;
  background: #ffffff;
...
.qanim-scene-card.visible {
  display: block;
}
.qanim-scene-card.revealed {
  opacity: 1;
...
#qanim-scene7 .formula-text {
  font-family: 'Segoe UI', system-ui, -apple-system, monospace, sans-serif;
  font-size: 28px;
  font-weight: 800;
  color: #1e293b;
...
#qanim-scene7 .var-symbol {
  font-family: 'Segoe UI', system-ui, -apple-system, sans-serif;
  font-size: 22px;
  font-weight: 900;
  line-height: 1;
}
#qanim-scene7 .var-label {
  font-size: 11px;
  font-weight: 700;
  text-transform: uppercase;
...
#qanim-scene7 .key-note-strip::before {
  content: '\26A1';
  font-size: 16px;
  flex-shrink: 0;
  color: #d97706;
...
/* ── Scene 8: Step by Step ── */
#qanim-scene8 .steps-outer {
  background: linear-gradient(135deg, #f8f9ff 0%, #f3f6ff 100%);
  border: 1.5px solid #e2e8f0;
  border-radius: 14px;
  padding: 20px 20px 8px;
}
#qanim-scene8 .step-row {
  display: flex;
  align-items: flex-start;
  gap: 18px;
  padding: 16px 18px;
...
#qanim-scene8 .step-content {
  flex: 1;
}
#qanim-scene8 .step-title {
  font-family: 'Segoe UI', system-ui, -apple-system, sans-serif;
  font-size: 14px;
  font-weight: 700;
  color: #1e293b;
...
#qanim-scene8 .step-eq {
  font-family: 'Segoe UI', system-ui, -apple-system, monospace, sans-serif;
  font-size: 13px;
  color: #475569;
  line-height: 1.5;
}
#qanim-scene8 .try-note {
  font-family: 'Segoe UI', system-ui, -apple-system, sans-serif;
  font-size: 13px;
  font-weight: 600;
  color: #2563eb;
...
def _build_scene7_html(big_idea: dict) -> str:
    """Build the HTML content for Scene 7 — The Big Idea."""
    formula_name    = html_module.escape(big_idea.get("formula_name", "Core Formula"))
    formula_display = html_module.escape(big_idea.get("formula_display", "F = ma"))
    formula_insight = html_module.escape(big_idea.get("formula_insight", ""))
    key_note        = html_module.escape(big_idea.get("key_note", ""))
    variables       = big_idea.get("variables", [])
    ...
    for idx, var in enumerate(variables[:6]):
        ...
        label     = html_module.escape(str(var.get("label", "")))
        meaning   = html_module.escape(str(var.get("meaning", "")))
        chips_html += (
            '<div class="var-chip" style="'
            f'background:{palette["bg"]};border-color:{palette["border"]};'
            f'color:{palette["text"]};">'
            f'<div class="var-arrow" style="color:{palette["arrow"]};border-left-color:{palette["arrow"]};"></div>'
            f'<div class="var-symbol">{symbol}</div>'
            f'<div class="var-label">{label}</div>'
            f'<div class="var-meaning">{meaning}</div>'
            '</div>\n'
        )

    insight_html = (
        f'<div class="formula-insight">{formula_insight}</div>' if formula_insight else ""
    )
    note_html = (
        f'<div class="key-note-strip">{key_note}</div>' if key_note else ""
    )

    return (
        '<div class="qanim-scene-card" id="qanim-scene7">\n'
        '  <div class="qanim-scene-header"><h2>&#x1F4A1; The Big Idea</h2></div>\n'
        '  <div class="qanim-scene-body">\n'
        '    <div class="formula-box">\n'
        f'      <div class="formula-text">{formula_display}</div>\n'
        f'      <div class="formula-name">{formula_name}</div>\n'
        '    </div>\n'
        f'    {insight_html}\n'
        '    <div class="vars-row">\n'
        f'      {chips_html}'
...
def _build_scene8_html(step_by_step: dict) -> str:
    """Build the HTML content for Scene 8 — How We Solve It Step by Step."""
    steps    = step_by_step.get("steps", [])
    try_note = html_module.escape(step_by_step.get("try_it_note", "Try it yourself!"))

    rows_html = ""
    for idx, step in enumerate(steps[:10]):
        palette   = _STEP_CARD_COLORS[idx % len(_STEP_CARD_COLORS)]
        title_esc = html_module.escape(str(step.get("title", f"Step {idx+1}")))
        eq_esc    = html_module.escape(str(step.get("equation", "")))
        # Mark last step's equation with the try-note in blue
        eq_content = eq_esc
        if idx == len(steps) - 1 and try_note:
            eq_content = (
                f'{eq_esc}'
                f'<br><span class="try-note">{try_note}</span>'
            )
        rows_html += (
            '<div class="step-row" style="'
            f'background:{palette["bg"]};border-color:{palette["border"]};">\n'
            f'  <div class="step-num" style="color:{palette["num"]};">{idx + 1}</div>\n'
            '  <div class="step-content">\n'
            f'    <div class="step-title">{title_esc}</div>\n'
            f'    <div class="step-eq">{eq_content}</div>\n'
            '  </div>\n'
            '</div>\n'
        )

    return (
        '<div class="qanim-scene-card" id="qanim-scene8">\n'
        '  <div class="qanim-scene-header"><h2>&#x1F4CB; How We Solve It &#x2014; Step by Step</h2></div>\n'
        '  <div class="qanim-scene-body">\n'
...
# JS that wires the two new scene dots into the existing step-indicator
# and shows/hides the scene cards when those dots are clicked.
_SCENES_78_JS = r"""
<script id="qanim-scenes78-js">
(function initScenes78(){
  'use strict';
  if(window.__qanimScenes78Init) return;
  window.__qanimScenes78Init = true;

  var SCENE7_ID = 'qanim-scene7';
  var SCENE8_ID = 'qanim-scene8';
  var DOT7_ID   = 'qanim-dot7';
  var DOT8_ID   = 'qanim-dot8';
  // Which scene card is currently shown (null = none)
  ...
  function _hideCard(cardId){
    var card = _el(cardId);
    if(!card) return;
    card.classList.remove('revealed');
    setTimeout(function(){
      // Only hide if still not revealed (animation may have re-triggered)
  ...
  function _setDotActive(dotId, active){
    var dot = _el(dotId);
    if(!dot) return;
    if(active){
  ...
  function _toggleScene(sceneId, dotId, otherSceneId, otherDotId){
    if(activeScene === sceneId){
      // clicking active scene — collapse it
      _hideCard(sceneId);
      _setDotActive(dotId, false);
      activeScene = null;
    } else {
      // Hide other scene if open
      if(activeScene){
        _hideCard(activeScene);
        // Deactivate whichever dot was active
        _setDotActive(DOT7_ID, false);
        _setDotActive(DOT8_ID, false);
      }
      _showCard(sceneId);
      _setDotActive(dotId, true);
      activeScene = sceneId;
    }
  }

  // Make scenes available in the wrapper div (placed after .dashboard)
  function _ensureCards(){
    var scene7 = _el(SCENE7_ID);
    var scene8 = _el(SCENE8_ID);
    // cards are already in the DOM, just ensure display:none initially
    if(scene7){ scene7.style.display = 'none'; scene7.classList.remove('revealed'); }
    if(scene8){ scene8.style.display = 'none'; scene8.classList.remove('revealed'); }
  }

  // Inject two pill dots into the step-indicator after existing dots
  function _injectDots(){
    var dotsContainer = document.getElementById('dots');
    if(!dotsContainer) return;
    if(_el(DOT7_ID)) return; // already injected

    // Connector + Scene 7 dot
    var conn1 = document.createElement('div');
    conn1.className = 'step-connector';

    var dot7 = document.createElement('div');
    dot7.className = 'step-dot';
    dot7.id = DOT7_ID;
    dot7.textContent = '\u1F4A1 Big Idea';
    dot7.title = 'The Big Idea — core formula';
    dot7.style.cursor = 'pointer';
    dot7.addEventListener('click', function(){
      _toggleScene(SCENE7_ID, DOT7_ID, SCENE8_ID, DOT8_ID);
    });

    // Connector + Scene 8 dot
    var conn2 = document.createElement('div');
    conn2.className = 'step-connector';

    var dot8 = document.createElement('div');
    dot8.className = 'step-dot';
    dot8.id = DOT8_ID;
    dot8.textContent = '\u1F4CB Steps';
    dot8.title = 'How We Solve It — step by step';
    dot8.style.cursor = 'pointer';
    dot8.addEventListener('click', function(){
      _toggleScene(SCENE8_ID, DOT8_ID, SCENE7_ID, DOT7_ID);
    });

    // Find the step-label div (last child of dots) and insert before it
    var stepLabel = dotsContainer.querySelector('#step-label');
    if(stepLabel){
      dotsContainer.insertBefore(conn1, stepLabel);
      dotsContainer.insertBefore(dot7, stepLabel);
      dotsContainer.insertBefore(conn2, stepLabel);
      dotsContainer.insertBefore(dot8, stepLabel);
    } else {
      dotsContainer.appendChild(conn1);
      dotsContainer.appendChild(dot7);
      dotsContainer.appendChild(conn2);
      dotsContainer.appendChild(dot8);
    }
  }
...
def inject_scenes_7_and_8(html: str, big_idea: dict, step_by_step: dict) -> str:
    """
    Inject Scene 7 (The Big Idea) and Scene 8 (Step by Step) into the
    animation HTML.

    Strategy:
      1. Inject CSS into <head>.
      2. Inject the two scene card <div>s into a wrapper after the main
         .dashboard div (inside <body>).  We find the end of the last
         .dashboard card via a balanced-tag scan.
      3. Inject the wiring JS before </body>.

    The scene cards are display:none by default; clicking the new pill
    dots in the step-indicator reveals them with a slide-in transition.
    """
    # Build HTML fragments
    scene7_html   = _build_scene7_html(big_idea)
    scene8_html   = _build_scene8_html(step_by_step)
    cards_wrapper = (
        '\n<!-- QAnim Scenes 7 & 8 -->\n'
        '<div id="qanim-scenes78-wrapper" style="width:100%;max-width:900px;'
    ...
    try:
        if '</head>' in html:
            html = html.replace('</head>', _SCENES_78_CSS + '\n</head>', 1)
    except Exception as exc:
        QAnimLogger.warn("Scenes78Injector", f"CSS injection failed: {exc}")

    # 2 — Cards wrapper: insert right after the outermost .dashboard div
    #     We use _insert_before_container_close on the <body> tag so the
    #     cards appear after the dashboard (inside body, after </div> of
    #     dashboard).  A simpler approach: find </body> and insert just
    #     before it alongside the JS; but we want the cards BETWEEN the
    #     dashboard and the floating panels, so we insert after the last
    #     .dashboard closing tag instead.
    try:
        # Find the last occurrence of class="dashboard" opening tag and
        # locate its matching closing </div> via balanced scan.
        dashboard_matches = list(re.finditer(r'<div[^>]+class=["\'][^"\']*dashboard[^"\']*["\']', html, re.IGNORECASE))
        if dashboard_matches:
            last_match = dashboard_matches[-1]
            tag_re = re.compile(r'<div\b[^>]*>|</div\s*>', re.IGNORECASE)
            depth  = 1
            insert_at = None
            for tm in tag_re.finditer(html, last_match.end()):
                g = tm.group(0).lower()
                if g.startswith('</div'):
                    depth -= 1
                else:
            ...
            if insert_at is not None:
                html = html[:insert_at] + '\n' + cards_wrapper + html[insert_at:]
            else:
                # Fallback: insert before </body>
                if '</body>' in html:
                    html = html.replace('</body>', cards_wrapper + '\n</body>', 1)
        else:
            if '</body>' in html:
                html = html.replace('</body>', cards_wrapper + '\n</body>', 1)
    except Exception as exc:
        QAnimLogger.warn("Scenes78Injector", f"Card DOM injection failed: {exc}")
        try:
            if '</body>' in html:
                html = html.replace('</body>', cards_wrapper + '\n</body>', 1)
        except Exception:
    ...
    try:
        if '</body>' in html:
            html = html.replace('</body>', _SCENES_78_JS + '\n</body>', 1)
        else:
            html += '\n' + _SCENES_78_JS
    except Exception as exc:
        QAnimLogger.warn("Scenes78Injector", f"JS injection failed: {exc}")

    QAnimLogger.ok("Scenes78Injector", "Scene 7 (Big Idea) + Scene 8 (Step by Step) injected")
    return html
...
if __name__ == "__main__":
    ...
    for cat, q in questions_to_test.items():
        ...
        print("  OK  All panels injected (Find/AnswerBox/Notes/Glossary)")
