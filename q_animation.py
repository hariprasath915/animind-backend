...
WHAT THIS VERSION DOES:
  ...
  - All panels, controls, glossary, notes, and ToFind are injected by Python.
...
steps_data_js: The var stepsData = [...] array — exactly 6 objects, one per step.
  Each object must have:
    label, blurOp, title, badges (HTML string), desc, layerOpacities (object), overlays (array).
  ...
  badges: HTML string like '<span class="badge badge-cyan">L = 1m</span>'.
    Allowed classes: badge-cyan, badge-orange, badge-green.
...
<g class="svg-layer" id="layer-summary" style="opacity:0">
  ...
  <text x="690" y="370" font-family="'Segoe UI',sans-serif" font-size="13" font-weight="800" fill="#15803d" text-anchor="middle">To Find: ?</text>
</g>"""
...
#tofind-backdrop{display:none;position:fixed;inset:0;z-index:8000;background:rgba(15,23,42,.40);backdrop-filter:blur(4px);opacity:0;transition:opacity .22s;}
...
.tf-close-btn{width:30px;height:30px;border-radius:8px;border:1px solid #e2e8f0;background:#f8fafc;color:#64748b;font-size:12px;display:flex;align-items:center;justify-content:center;cursor:pointer;}
...
.tofind-item{display:flex;align-items:flex-start;gap:12px;padding:12px 14px;border-radius:10px;background:#f8fafc;border:1px solid #e2e8f0;opacity:0;transform:translateX(-12px);}
...
(function initToFind(){
  ...
  function _el(id){return document.getElementById(id);}
  function _onReady(fn){if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',fn);else setTimeout(fn,0);}
  function _loadTargets(){try{var t=_el('__tofind_data__');if(!t)return[];var d=JSON.parse(t.textContent)||{};return Array.isArray(d.targets)?d.targets:[];}catch(e){return[];}}
  function _esc(t){return String(t).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');}
  function _build(targets){if(built)return;built=true;var c=_el('tofind-items-container');if(!c)return;if(!targets||!targets.length){c.innerHTML='<div class="tofind-empty">No specific targets detected.</div>';return;}var h='';for(var i=0;i<targets.length;i++){h+='<div class="tofind-item" id="tofind-item-'+i+'"><div class="tofind-check">&#10003;</div><div class="tofind-text">'+_esc(targets[i])+'</div></div>';}c.innerHTML=h;}
  function _animate(){var items=document.querySelectorAll('.tofind-item');for(var i=0;i<items.length;i++){(function(el,idx){el.style.opacity='0';el.style.transform='translateX(-12px)';el.style.transition='none';setTimeout(function(){el.style.transition='opacity .28s ease,transform .28s ease';el.style.opacity='1';el.style.transform='translateX(0)';},60+idx*80);})(items[i],i);}}
  function openToFind(){var bd=_el('tofind-backdrop'),pn=_el('tofind-panel');if(!bd||!pn)return;_build(_loadTargets());bd.classList.add('open');pn.classList.add('open');pn.setAttribute('aria-hidden','false');open=true;setTimeout(_animate,100);}
  function closeToFind(){var bd=_el('tofind-backdrop'),pn=_el('tofind-panel');if(bd)bd.classList.remove('open');if(pn){pn.classList.remove('open');pn.setAttribute('aria-hidden','true');}open=false;}
  window.openToFind=openToFind;window.closeToFind=closeToFind;window.toggleToFind=function(){open?closeToFind():openToFind();};
  _onReady(function(){
    ...
    var cb=_el('tofind-close');if(cb)cb.addEventListener('click',closeToFind);
    var bd=_el('tofind-backdrop');if(bd)bd.addEventListener('click',closeToFind);
    document.addEventListener('keydown',function(e){if(e.key==='Escape'&&open)closeToFind();});
  });
})();
</script>
"""
...
def assemble_html(question: str, scene: dict, sol: dict, svg_data: dict) -> str:
    ...
    # ToFind JSON
    tofind_json = json.dumps({"targets": to_find}, ensure_ascii=False)
