...
Given a mechanics/kinematics question, produce a complete structured
Explanation Script in the EXACT JSON format shown below.
...

{
  ...
  "given": [
    {"symbol": "<LaTeX symbol>", "value": "<numeric>", "unit": "<unit>", "description": "<label>"}
  ],
  ...
  "solution_steps": [
    {
      ...
      "substitution_latex": "<LaTeX with numbers substituted>",
      "result_latex": "<LaTeX result>",
      "result_numeric": "<plain numeric + unit string>"
    }
  ],
  "final_answer": {
    "symbol": "<symbol>",
...
def generate_explanation_script(model, question: str) -> dict:
    """Part 1: Ask Gemini to produce a structured explanation script."""
...
Given a structured Explanation Script (JSON), produce a Step-by-Step
Animation Sequence in the EXACT JSON format below.
Return ONLY valid JSON — no markdown, no preamble, no trailing text.

Rules:
- Always produce exactly 4 scenes (Scene 0 … Scene 3).
- Scene 0  → frame/setup reveal.
- Scene 1  → driver element focus (blur background).
- Scene 2  → full mechanism running (slight blur).
- Scene 3  → freeze at key angle/state + show formula box.
- badge_color must be one of: "cyan", "orange", "green".
- freeze_target: null for scenes that keep running; a number (radians or
  normalized 0-1) for the final freeze scene.

{
  "mechanism_type": "<e.g. slider_crank | gear_mesh | four_bar | belt_pulley>",
  "canvas_notes": "<brief description of what the SVG stage should look like>",
  "scenes": [
    {
      "index": 0,
      "label": "<SHORT CAPS label for dot indicator>",
      "title": "<Step N: Title>",
      "description": "<2-3 sentence explanation shown in the info card>",
      "badges": [{"text": "<badge text>", "color": "cyan|orange|green"}],
      "blur_level": 0.0,
      "crank_visible": false,
      "linkage_visible": false,
      "overlay_items": [
        {
          "type": "param_box",
...
Given:
  • An Explanation Script (Part 1 JSON)
  • An Animation Sequence (Part 2 JSON)

Produce the COMPLETE BODY of an SVG scene — specifically the content
that goes INSIDE <svg id="stage" viewBox="0 0 850 450"> … </svg>.

This includes:
  1. <defs> block (gradients, filters, markers, patterns).
  2. Static structural layers (frame, guides, ground hatching, centre axis).
  3. Animated component layers (crank, connecting rod, piston — or gears, etc.)
     wrapped in <g class="svg-layer" id="layer-…" style="opacity:0;">.
  4. A blur-shield rect: <rect id="blur-shield" … opacity="0" …/>.
  5. Overlay groups for each scene (id="overlay-step0" … "overlay-step3")
     containing labels, dimension boxes, rotation arrows, angle arcs,
     velocity vectors, formula boxes etc.
...
Style conventions MUST match the reference design:
  • Background grid: <pattern id="grid" width="40" height="40">
  • Dark metallic gradient: id="darkMetal" (#2a2a30 → #0f0f13)
  • Steel gradient:         id="steel"     (#fff → #c0c0c0 → #707070 → #303030)
  • Drop shadow filter:     id="dropShadow"
  • Cyan glow filter:       id="glowCyan"
  • Cyan arrow marker:      id="arrowCyan"   fill="#66fcf1"
  • Red arrow marker:       id="arrowRed"    fill="#ff3366"
  • All animated crank/rod elements must have id attributes that match the JS.
  • Piston group: <g id="piston-group" transform="translate(…)">
  • Velocity vector group: <g id="velocity-vector">
  • Text labels use fill="#fff" or fill="#66fcf1" / "#fca311" / "#ff3366"
  • Formula boxes: dark rect + green (#97c459) border + monospace text rows
"""
...
Produce the COMPLETE JavaScript that:
  1. Defines CONFIG object (scaled pixel dimensions from real dimensions).
  2. Implements calculatePositions(angle) → {ax, ay, bx, by} for the mechanism.
  3. Implements updateDOM(positions) to move all SVG elements.
  4. Implements animationLoop() using requestAnimationFrame.
  5. Defines stepsData[] array — one entry per scene — with:
       label, crankOp, rodPistonOp, blurOp, overlays[], freezing, startAnim,
       title, badges (HTML string), desc (HTML string).
  6. Implements applyStep(stepIdx), nextStep(), resetAnim().
  7. Calls setTimeout(() => resetAnim(), 100) at the end.

Return ONLY the raw JavaScript code — no <script> tags, no markdown fences.

IMPORTANT rules:
  • Layer IDs must match those in the SVG: layer-crank, layer-rod-piston,
    blur-shield, overlay-step0 … overlay-step3.
  • For the freeze scene: set isFreezing=true and smoothly ease theta toward
    the target angle (Math.PI/3 for 60°, etc.) using the easing pattern:
      let diff = target - (theta % (2*Math.PI)); … theta += diff * 0.05;
  • SVG Y-axis is inverted: ay = cy - r*sin(angle).
  • Piston X: bx = cx + r*cos(angle) + sqrt(l² - (r*sin(angle))²).
  • Badge HTML must use: <span class="badge badge-cyan|badge-orange|badge-green">
"""
...
def _build_dots_html(num_steps: int) -> str:
    dots = "\n".join(
        f'                <div class="step-dot{"  active" if i == 0 else ""}"></div>'
...
def assemble_html(
    ...
    num_scenes = len(sequence.get("scenes", [])) or 4
    dots_html = _build_dots_html(num_scenes)
